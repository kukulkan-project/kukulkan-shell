...
public class HerokuCommands extends AbstractCommand {
    ...
    @Autowired
    private NativeCommandContext nativeCommandContext;
    ...
    public Availability addHerokuSupportAvailability() {
        if (!nativeCommandContext.getAvailableCommands().containsKey(HEROKU)) {
            return Availability.unavailable(String.format("%s must be installed", HEROKU));
        }
    ...
    @Bean
    public NativeCommandProvided herokuCommand() {
        return () -> Optional.of(new DefaultNativeCommand(HEROKU, HEROKU + " version"));
    }
