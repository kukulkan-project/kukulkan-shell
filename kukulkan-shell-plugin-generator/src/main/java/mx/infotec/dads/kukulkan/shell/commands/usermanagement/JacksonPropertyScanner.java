 ...
 * 
 * A {@link PropertyScanner} that uses a Jackson configuration to inspect the
 * properties of a type. This allows to document the properties the same way
 * they are serialized when using the given {@link ObjectMapper}, and is
 * especially useful when used with Spring's configuration.
 */
public class JacksonPropertyScanner implements PropertyScanner {
    ...
    /**
     * Creates a new {@link JacksonPropertyScanner} based on the given
     * {@link ObjectMapper}'s configuration.
     *
     * @param mapper
     *            the Jackson {@link ObjectMapper} that is used to serialize the
     *            objects to document
     */
    ...
    public List<Property> getProperties(Type type) {
        return getJacksonProperties(type).stream().filter(JacksonPropertyScanner::isReadable).map(this::convertProp)
                .collect(toList());
    }
